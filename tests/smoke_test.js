...
check('production multi-row editor', appHtml.includes('production-batch-add') && appHtml.includes('collectProductionBatchRows') && appHtml.includes("callServerWithRetry('batchCreateProductionRows'") && styles.includes('.production-batch-row'), 'production bulk entry UI is incomplete');
...
check('warehouse flower logistics schemas', config.includes('DEPARTMENT_OPERATION_TABLE_SCHEMAS') && ['warehouse_tasks','logistics_tasks','flower_project_plans','flower_tasks','flower_tool_orders','flower_handoffs','flower_materials'].every((name) => config.includes(name)), 'department operation tables are incomplete');
check('department operation lazy bundle', code.includes('function getProjectDepartmentOperations(') && appHtml.includes("callServer('getProjectDepartmentOperations'") && appHtml.includes('operationByProject'), 'warehouse/flower/logistics data is not loaded once per department');
check('department operation batch persistence', code.includes('function batchCreateDepartmentOperationRows(') && appHtml.includes("callServerWithRetry('batchCreateDepartmentOperationRows'") && appHtml.includes('operation-batch-add'), 'department multi-row save is incomplete');
check('department operation inline status', appHtml.includes('data-operation-inline') && appHtml.includes('updateOperationInlineField') && appHtml.includes('DEPARTMENT_OPERATION_INLINE_FIELDS'), 'department statuses are not editable inline');
check('batch retry with stable request id', appHtml.includes('callServerWithRetry') && appHtml.includes('retryableRpcError') && appHtml.includes('batchFingerprint') && appHtml.includes('_request_id:editor.requestId'), 'batch retry does not preserve a stable idempotency key');
check('batch retry prevents duplicate rows', config.includes('BATCH_IDEMPOTENCY_SCHEMA') && dataStore.includes("{ batch_request_id: batchRequestId }") && dataStore.includes('replayed: true'), 'server batch retry can write duplicate rows');
check('batch save avoids redundant reload', appHtml.includes('mergeBatchRows') && !/async function saveOperationBatch[\s\S]*?await loadProjectOperations/.test(appHtml) && !/async function saveProductionBatch[\s\S]*?await loadProjectProduction/.test(appHtml), 'batch save still performs a second full reload RPC');
check('batch timing diagnostics', code.includes('write_audit_ms') && code.includes('prepare_ms') && appHtml.includes("'[WeddingOps RPC:batch]'"), 'batch timing diagnostics are missing');
